...
class _QuoteRequestPageState extends State<QuoteRequestPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await GetIt.I<ApiClient>().cloud('v1-quote-requests-create', {
        ...
        'source': 'Site',
        'photoNames': _photos.map((e) => e.name).toList(),
      });
  ...
  Widget _success() => const Column(
        ...
        children: [
          Icon(Icons.task_alt, size: 64, color: AppColors.gold),
          SizedBox(height: 16),
          Text('Solicitação enviada', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w800)),
          SizedBox(height: 8),
          Text(
        ...
        child: Column(
          ...
          children: [
            Image.asset(
              ...
              height: 105,
              fit: BoxFit.contain,
              errorBuilder: (_, __, ___) => const Icon(Icons.format_paint, size: 60, color: AppColors.gold),
            ),
            const SizedBox(height: 18),
            const Text('Solicitar orçamento', textAlign: TextAlign.center, style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Medidas / metragem (se souber)', prefixIcon: Icon(Icons.straighten)),
            ),
